import { renderSolarField, renderTerrainRelief, solarAltitude } from '../astronomy/solar-field.js?v=phase1c51-rollback';
import { renderMapGrid } from './map-grid.js?v=phase1c20';
...
(() => {
  ...
  const mapLinework = document.querySelector('#mapLinework');
  ...
  // Derive a line-only overlay from the exact SVG currently displayed. The
  // body fill stays beneath the solar field; all official boundaries and
  // coastlines are redrawn above it without introducing another map source.
  async function loadLineworkOverlay() {
    if (!mapLinework || mapLinework.src) return;
    try {
      const response = await fetch('assets/map/svg/presentation-coastline.svg?v=phase1c51-rollback', { cache: 'no-store' });
      const source = await response.text();
      const doc = new DOMParser().parseFromString(source, 'image/svg+xml');
      doc.querySelector('#path3')?.remove();
      const blob = new Blob([new XMLSerializer().serializeToString(doc)], { type: 'image/svg+xml' });
      mapLinework.src = URL.createObjectURL(blob);
    } catch {
      mapLinework.remove();
    }
  }
  ...
  // All visual children share the untransformed map-visual coordinate space.
  // The parent applies the single presentation scale, so SVG, Canvas and
  // markers cannot drift apart because of independent transformed rectangles.
  function mapLayoutFrame() {
    const parent = mapImage.parentElement;
    const rect = parent.getBoundingClientRect();
    return { left: rect.left, top: rect.top, width: mapImage.offsetWidth || parent.offsetWidth, height: mapImage.offsetHeight || parent.offsetHeight };
  }
  function syncMarkers() { const frame=mapLayoutFrame(); if(!frame.width||!frame.height)return; const sr=VIEWBOX.width/VIEWBOX.height, br=frame.width/frame.height, scale=br>sr?frame.height/VIEWBOX.height:frame.width/VIEWBOX.width, ox=(frame.width-VIEWBOX.width*scale)/2, oy=(frame.height-VIEWBOX.height*scale)/2; cityLayer.querySelectorAll('.city-marker').forEach((m)=>{const p=m._point;m.style.left=`${ox+p.x*scale}px`;m.style.top=`${oy+p.y*scale}px`;m.classList.toggle('selected',state.selected.includes(m.dataset.name));}); }
  ...
  load().then(points=>{state.points=points.length?points:fallback;placeMarkers(state.points);ensureStageMarkup();ensureCompareDetails();migrateSeasonControls();ensureOverview();setup();loadLineworkOverlay();state.selected=[];state.userSelected=false;setMode('dawn-progress');render();});mapImage.addEventListener('load',render);window.addEventListener('resize',render,{passive:true});
})();
